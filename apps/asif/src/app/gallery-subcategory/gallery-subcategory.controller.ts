...
export class Controller {
  ...
  async Add(req: any, res: any, next: any) {
    const data = req.body;
    try {
      const Create = await prisma.gallerySubcategory.create({ data: data })
      res.status(200).json({
        success: true,
        code: 200,
  ...
  async Upadte(req: any, res: any, next: any) {
    ...
    try {
      if (params.id !== '' && params.id !== null && params.id !== undefined) {
        ...
        const Update = await prisma.gallerySubcategory.update({ data, where })
        res.status(200).json({
          success: true,
          code: 200,
  ...
  async Delete(req: any, res: any, next: any) {
    ...
    try {
      if (params.id !== '' && params.id !== null && params.id !== undefined) {
        const where = { slug: params.id }
        const Detele = await prisma.gallerySubcategory.delete({ where })
        res.status(200).json({
          success: true,
          code: 200,
