...
const foldername = "homeAdvantage";
...
export class Controller {
  ...
  async Add(req: any, res: any, next: any) {
    ...
    try {
      ...
      const Create = await prisma.homeAdvantage.create({ data: data })
      res.status(200).json({
        success: true,
        code: 200,
  ...
  async Upadte(req: any, res: any, next: any) {
    ...
    try {
      if (params.id !== '' && params.id !== null && params.id !== undefined) {
        ...
        const Update = await prisma.homeAdvantage.update({ data, where })
        res.status(200).json({
          success: true,
          code: 200,
  ...
  async Delete(req: any, res: any, next: any) {
    ...
    try {
      if (params.id !== '' && params.id !== null && params.id !== undefined) {
        const where = { slug: params.id }
        const Detele = await prisma.homeAdvantage.delete({ where })
        res.status(200).json({
          success: true,
          code: 200,
