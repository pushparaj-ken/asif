...
export class Controller {
  async Add(req: any, res: any, next: any) {
    const data = req.body;
    try {
      const Create = await prisma.planning.create({ data: data })
      res.status(200).json({
        success: true,
        code: 200,
  ...
  async Upadte(req: any, res: any, next: any) {
    ...
    try {
      if (params.id !== '' && params.id !== null && params.id !== undefined) {
        ...
        const Update = await prisma.planning.update({ data, where })
        res.status(200).json({
          success: true,
          code: 200,
  ...
  async Delete(req: any, res: any, next: any) {
    ...
    try {
      if (params.id !== '' && params.id !== null && params.id !== undefined) {
        const where = { slug: params.id }
        const Detele = await prisma.planning.delete({ where })
        res.status(200).json({
          success: true,
          code: 200,
  ...
  async Details(req: any, res: any, next: any) {
    try {
      ...
      const List = await prisma.planning.findMany({
