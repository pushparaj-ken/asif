...
export class Controller {
  async Add(req: any, res: any, next: any) {
    const data = req.body;
    try {
      const Create = await prisma.instructorSection.create({ data: data })
  ...
  async Upadte(req: any, res: any, next: any) {
    ...
    try {
      if (params.id !== '' && params.id !== null && params.id !== undefined) {
        ...
        const Update = await prisma.instructorSection.update({ data, where })
        return await responseService.apiSuccessResponse(res, null);
      } else {
  ...
  async Delete(req: any, res: any, next: any) {
    ...
    try {
      if (params.id !== '' && params.id !== null && params.id !== undefined) {
        const where = { slug: params.id }
        const Detele = await prisma.instructorSection.delete({ where })
        return await responseService.apiSuccessResponse(res, null);
      } else {
        return await responseService.apiFailResponse(res, 'Id is Required');
      }
